...
const VerificationResult = ({ data: initialData, code, onBack }) => {
  const [data, setData] = useState({ ...initialData, email: initialData.email || 'Not provided', additionalInfo: initialData.additionalInfo || 'No additional metadata available.' });
  const isApproved = data.status === 'Approved';

  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-6 md:py-10 animate-[fadeIn_0.5s_ease-out] flex flex-col min-h-[80vh]">
      ...
      <div className="w-full bg-gradient-to-r from-white/[0.05] to-transparent backdrop-blur-2xl border border-white/[0.08] rounded-3xl p-6 md:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.3)] mb-6 relative overflow-hidden flex flex-col md:flex-row gap-8 items-center md:items-start">
        ...
        <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-black/50 border border-white/10 flex items-center justify-center shadow-inner flex-shrink-0 relative overflow-hidden">
          <div className="text-4xl text-gray-500 font-bold">{data.primaryName?.charAt(0).toUpperCase()}</div>
          <div className={`absolute bottom-0 w-full h-1/4 ${isApproved ? 'bg-green-500/20' : 'bg-orange-500/20'} blur-md`}></div>
        </div>
        <div className="flex-1 text-center md:text-left w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            ...
            <div className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border backdrop-blur-md self-center md:self-start shadow-lg ${isApproved ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-orange-500/10 border-orange-500/30 text-orange-500'}`}>
              <span className={`w-2 h-2 rounded-full animate-pulse ${isApproved ? 'bg-green-400' : 'bg-orange-500'}`}></span>
              <span className="text-sm font-bold uppercase tracking-wider">{data.status}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="bg-[#080808] border border-white/5 rounded-2xl p-6 md:p-8 flex-1 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg text-gray-200 font-medium">{data.date1} {data.date2 ? `- ${data.date2}` : ''}</p>
              </div>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-6">
          <div className="bg-[#050505] border border-white/5 rounded-2xl p-6 shadow-inner">
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-white/5 flex items-center gap-2 text-xs text-gray-500">
                 <svg className="w-4 h-4 text-green-500/70" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M2.166 4.999A11.954 11.954 0 0010 1.944 11.954 11.954 0 0017.834 5c.11.65.166 1.32.166 2.001 0 5.225-3.34 9.67-8 11.317C5.34 16.67 2 12.225 2 7c0-.682.057-1.35.166-2.001zm11.541 3.708a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                 End-to-End Encrypted via TIESVERSE
              </div>
            </div>
          </div>
        </div>
...
function App() {
  ...
  const [adminFormData, setAdminFormData] = useState({ field1: '', field2: '', field3: '', field4: '' });
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        const data = rows.slice(1).map(row => {
          ...
          return { field1: values[0]||'', field2: values[1]||'', field3: values[2]||'', field4: values[3]||'' };
        });
        resolve(data);
      };
  ...
  const handleAdminSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setAdminFormData({ field1: '', field2: '', field3: '', field4: '' });
        setCsvFile(null);
  ...
  // --- 🆕 NEW HANDLER: ADMIN QUICK SEARCH ---
  const handleAdminQuickSearch = async (e) => {
    ...
    try {
      const response = await fetch(API_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'search_users', query: adminSearchQuery }),
        headers: { 'Content-Type': 'text/plain;charset=utf-8' }
      });
      const result = await response.json();
      if (result.success) {
        setAdminSearchResults(result.results);
      } else {
  ...
  // --- UI UPGRADE: PREMIUM DYNAMIC FORM INPUTS ---
  ...
  const renderCsvInstructions = () => {
    ...
    if(activeCategory === 'Internship') cols = "Name, Role, Start Date, End Date";
    ...
    <div className="relative min-h-screen bg-black text-white font-sans overflow-x-hidden selection:bg-orange-500/30 flex flex-col pb-8 md:pb-12">
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 py-6 max-w-[1400px] mx-auto w-full lg:px-10">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-300">
          <button onClick={resetToHome} className="text-white hover:text-orange-400 transition-colors">Home</button>
          <button className="hover:text-white transition-colors">How It Works</button>
          <button className="hover:text-white transition-colors">Lookup</button>
        </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col w-full">
        ...
        {currentView === 'home' && (
          <div className="animate-[fadeIn_0.4s_ease-out] flex-1 flex flex-col">
            <main className="relative z-10 flex flex-col items-center text-center mt-12 md:mt-20 px-4">
              <h2 className="text-2xl md:text-3xl font-bold tracking-wide">
                Official <span className="text-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.4)]">Verification</span> System
              </h2>
              <p className="text-gray-400 text-sm md:text-base mt-3 max-w-lg">
                Generate and verify credentials securely across the network.
              </p>

              <h1 className="text-5xl sm:text-6xl md:text-[80px] font-extrabold mt-16 md:mt-24 leading-[1.1] tracking-tight max-w-5xl">
              ...
              <button className="mt-10 px-8 py-3.5 rounded-full border border-orange-500 text-white text-lg font-medium bg-black/50 hover:bg-orange-500/10 transition-all shadow-[0_0_20px_rgba(249,115,22,0.2)] hover:shadow-[0_0_30px_rgba(249,115,22,0.4)] backdrop-blur-md">
                Get Started
              </button>
            ...
            <section className="relative z-10 max-w-[1400px] w-full mx-auto px-6 lg:px-10 mt-20 mb-20 grid grid-cols-1 xl:grid-cols-12 gap-6 items-end">
              <div className="xl:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="bg-[#080808] border border-white/5 rounded-2xl p-6 transition-all hover:border-orange-500/20 hover:bg-[#0c0c0c]">
              ...
              <div className="xl:col-span-4 flex flex-col gap-4 w-full">
                <div className="bg-[#080808] border border-white/5 rounded-2xl p-5 w-full">
            ...
            <div className="bg-[#080808]/90 backdrop-blur-3xl border border-white/[0.05] rounded-3xl p-6 md:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden mb-12">
              ...
              <form onSubmit={handleAdminQuickSearch} className="flex gap-3 mb-8">
                <input 
                  ...
                  placeholder="Search by Name, Role, or Unique Code..." 
                  className="flex-1 bg-[#050505] border border-white/[0.05] rounded-xl px-5 py-4 text-white text-sm focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/30 transition-all shadow-inner placeholder-gray-600" 
                />
                  ...
                  {adminSearchResults.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {adminSearchResults.map((user, idx) => (
                        <div key={idx} className="bg-[#050505] border border-white/[0.05] p-5 rounded-2xl hover:border-orange-500/30 transition-colors group relative overflow-hidden">
                          ...
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              ...
                              <h4 className="text-lg font-bold text-white mt-2">{user.primaryName}</h4>
                            </div>
                            <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-md ${user.status === 'Approved' ? 'text-green-400 bg-green-400/10 border border-green-400/20' : 'text-orange-400 bg-orange-400/10 border border-orange-400/20'}`}>
                              {user.status}
                            </span>
                          </div>
                          
                          <div className="space-y-1.5 text-xs text-gray-400 font-mono">
                            <p><span className="text-gray-600 mr-2">ROLE:</span>{user.secondaryRole}</p>
                            <p><span className="text-gray-600 mr-2">CODE:</span><span className="text-white tracking-widest">{user.code}</span></p>
                            <p><span className="text-gray-600 mr-2">DATE:</span>{user.date1} {user.date2 ? ` - ${user.date2}` : ''}</p>
                          </div>
                        </div>
                      ))}
                    </div>

...
// Live deployment URL
const API_URL = "https://script.google.com/macros/s/AKfycbzk_HxmdyL3nH653_OYBTioj0WZcJmyaWJC8IL3yvgv1dkjxBLBqCRt91kDfuRkRbOb/exec";
...
// =====================================================================
// 🔒 CUSTOM COMPONENT: Strict Date Picker Input (Kills Manual Typing)
// =====================================================================
const StrictDateInput = React.forwardRef(({ value, onClick, placeholder, disabled, isTime }, ref) => (
  <input
    className={`w-full bg-[#050505] border border-white/[0.05] rounded-xl px-4 py-3.5 text-white text-sm focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/30 transition-all shadow-inner placeholder-gray-700 ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
    onClick={onClick}
    onKeyDown={(e) => e.preventDefault()} // Strictly blocks typing
    value={value}
    ref={ref}
    placeholder={placeholder}
    readOnly={!isTime} // Force popup click for dates
    disabled={disabled}
...
// =====================================================================
// 📄 COMPONENT: Full-Screen Result Page UI (Public View)
// =====================================================================
const VerificationResult = ({ data, code, onBack }) => {
  ...
  if (data.date2 && String(data.date2).toLowerCase() !== 'present') {
    displayDate += ` to ${formatDate(data.date2)}`;
  } else if (String(data.date2).toLowerCase() === 'present') {
    displayDate += ` to Present`;
  }
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="bg-[#080808] border border-white/5 rounded-2xl p-5 md:p-8 flex-1 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {data.lead && (
                <div className="space-y-2">
                  <label className="text-xs text-gray-500 uppercase tracking-wider font-medium">Reporting To</label>
            ...
            {data.pdfLink && (
              <div className="mt-8 pt-6 border-t border-white/5">
                <a href={data.pdfLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-orange-500/10 border border-orange-500/30 rounded-xl text-orange-500 font-bold uppercase tracking-widest hover:bg-orange-500/20 transition-all text-xs">
...
function App() {
  ...
  const [adminFormData, setAdminFormData] = useState({
    gender: 'Male', name: '', role: ROLES_LIST[0], dateStart: null, dateEnd: null, lead: ''
  });
  ...
  // Custom Date Handler for DatePicker Component
  const handleDateChange = (field, date) => {
    setAdminFormData({ ...adminFormData, [field]: date });
  };
  ...
  const parseCSV = (file) => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        if (rows.length < 2) { reject("CSV file is empty or missing headers."); return; }
        const data = rows.slice(1).map(row => {
          const values = row.split(',').map(v => v.trim());
          return { gender: values[0]||'', name: values[1]||'', role: values[2]||'', dateStart: values[3]||'', dateEnd: values[4]||'', lead: values[5]||'' };
        });
        resolve(data);
      };
  ...
  const handleAdminSubmit = async (e) => {
    ...
    if (activeTab === 'single') {
        if (!adminFormData.name || !adminFormData.role || !adminFormData.dateStart) {
            setMessage("Error: Name, Role, and Start/Issue Date are mandatory.");
            setLoading(false); return;
        }
        if (activeCategory === 'Offer Letter' && (!adminFormData.dateEnd || !adminFormData.lead)) {
            setMessage("Error: Joining Date and Manager Name are mandatory for Offer Letters.");
            setLoading(false); return;
        }
    }
    ...
    try {
      ...
      if (activeTab === 'single') { 
        // Convert JS Dates to YYYY-MM-DD string before sending to backend API
        const formatDateForBackend = (dateObj) => {
            if (!dateObj) return '';
            // Ensure local timezone formatting doesn't shift the day backwards
        ...
        payload.data = { 
            ...adminFormData, 
            dateStart: formatDateForBackend(adminFormData.dateStart),
            dateEnd: isCurrentlyEmployed ? 'Present' : formatDateForBackend(adminFormData.dateEnd) 
        };
      } else {
        if (!csvFile) { setMessage("Please select a CSV file."); setLoading(false); return; }
        try { payload.data = await parseCSV(csvFile); } catch(csvErr) { setMessage(`CSV Error: ${csvErr}`); setLoading(false); return; }
      }
      ...
      if (result.success) {
        ...
        setAdminFormData({ gender: 'Male', name: '', role: ROLES_LIST[0], dateStart: null, dateEnd: null, lead: '' });
        setIsCurrentlyEmployed(false);
        setCsvFile(null);
        if (fileInputRef.current) fileInputRef.current.value = "";
      } else { setMessage(result.error || 'Upload failed.'); }
    } catch (error) { setMessage('Network error. Check console.'); }
    setLoading(false);
  };
  ...
  const handleAdminQuickSearch = async (e) => {
    ...
    setIsSearchingAdmin(true); setHasSearchedAdmin(true);
    try {
  ...
  const handleGenerateCertificate = async (userCode) => {
    ...
    try {
      const response = await fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'generate_certificate', code: userCode }), headers: { 'Content-Type': 'text/plain;charset=utf-8' } });
      const result = await response.json();
      if (result.success) {
        setAdminSearchResults(prevResults => prevResults.map(u => u.code === userCode ? { ...u, pdfLink: result.url } : u));
        alert(`Success! PDF generated for ${userCode}`);
      } else { alert("Failed to generate: " + result.error); }
    } catch (error) { alert("Network error during generation."); }
  ...
  const renderUnifiedForm = () => {
    const isSharedCategory = activeCategory === 'Internship' || activeCategory === 'Member Certificate';
    ...
    return (
      <div className="space-y-5">
        ...
        <div>
          ...
          {isSharedCategory ? (
            <select name="role" value={adminFormData.role} onChange={handleAdminInputChange} required className={`${inputClass} text-gray-300 appearance-none cursor-pointer`}>
              {ROLES_LIST.map((r, i) => <option key={i} value={r}>{r}</option>)}
            </select>
          ) : (
            <input type="text" name="role" value={adminFormData.role} onChange={handleAdminInputChange} required className={inputClass} placeholder={isOfferLetter ? 'e.g., Software Engineer' : ''} />
          )}
        </div>

        {/* 🚀 STRICT CALENDAR DATE PICKERS APPLIED HERE */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 relative z-50">
          <div>
            ...
            <DatePicker 
                selected={adminFormData.dateStart} 
                onChange={(date) => handleDateChange('dateStart', date)} 
                dateFormat="dd/MM/yyyy"
                customInput={<StrictDateInput placeholder="DD/MM/YYYY" />}
                required
            />
          </div>
          <div>
            ...
            {isWebinar ? (
              ...
              <DatePicker 
                  selected={adminFormData.dateEnd} 
                  onChange={(date) => handleDateChange('dateEnd', date)} 
                  minDate={adminFormData.dateStart} // Logic: End Date >= Start Date
                  dateFormat="dd/MM/yyyy"
                  customInput={<StrictDateInput placeholder="DD/MM/YYYY" />}
                  required
              />
              ...
              <div>
                <DatePicker 
                  ...
                  minDate={adminFormData.dateStart} // Logic: End Date >= Start Date
                  dateFormat="dd/MM/yyyy"
                  customInput={<StrictDateInput disabled={isCurrentlyEmployed} placeholder="DD/MM/YYYY" />}
  ...
  const renderCsvInstructions = () => {
    let cols = "Gender (Male/Female), Name, Role, StartDate, EndDate, Lead";
    if (activeCategory === 'Webinar') cols = "Title, Author, Date, Time";
    else if (activeCategory === 'Offer Letter') cols = "Ignore(LeaveBlank), Name, Job Title, IssuanceDate, JoiningDate, ManagerName";
    return <span className="text-[10px] text-gray-500 mt-3 font-mono tracking-widest uppercase text-center block">Required Columns: <br/>{cols}</span>;
  };
  ...
  return (
    <div className="relative min-h-screen bg-black text-white font-sans overflow-x-hidden selection:bg-orange-500/30 flex flex-col pb-8 md:pb-12">
      ...
      {/* GLOBAL CSS OVERRIDES FOR REACT-DATEPICKER DARK THEME */}
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 py-6 max-w-[1400px] mx-auto w-full lg:px-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setCurrentView(isAdminAuth ? 'admin_dashboard' : 'admin_login')} className={`px-5 py-2 text-sm rounded-full border transition-all duration-300 bg-transparent ${currentView.includes('admin') ? 'border-orange-500/50 text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.15)]' : 'border-gray-600 hover:border-white text-white'}`}>
                ...
                <div className="flex flex-col gap-4 w-full h-full justify-end">
                  <div className="bg-[#080808] border border-white/5 rounded-2xl p-5 flex-1 flex flex-col justify-center w-full">
                    <form onSubmit={handleLookupSubmit} className="flex gap-2">
                      ...
                      <button type="submit" disabled={searchLoading} className="bg-white/5 hover:bg-white/10 border border-white/5 text-white text-sm px-5 py-3 rounded-xl transition-all disabled:opacity-50">{searchLoading ? '...' : 'Search'}</button>
                    </form>
                    {searchError && <p className="text-red-400 text-xs mt-3 text-center">{searchError}</p>}
                  </div>
                  <div className="bg-[#080808] border border-white/5 p-4 rounded-xl text-center"><p className="text-[11px] text-gray-600 leading-relaxed">Sign up for users to get lose the TIESVERSE<br />premium, and uncluttered.</p></div>
                </div>
              </div>
              <div className="xl:col-span-4 flex flex-col gap-4 w-full h-full justify-end">
                 <div className="bg-[#080808] border border-white/5 rounded-2xl p-6 transition-all hover:border-orange-500/20 hover:bg-[#0c0c0c] flex flex-col h-full"><div className="w-10 h-10 rounded-lg bg-[#111] border border-white/5 flex items-center justify-center mb-5 text-orange-500"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg></div><h3 className="text-white text-base font-semibold mb-2">Unique 10-Digit Code</h3><p className="text-gray-500 text-sm leading-relaxed">Receive a globally unique identifier instantly.</p></div>
              </div>
            </section>
          </div>
        )}

        {currentView === 'result' && searchResult && <div className="max-w-7xl mx-auto w-full px-4 md:px-6"><VerificationResult data={searchResult} code={searchCode} onBack={resetToHome} /></div>}
          ...
          <div className="animate-[fadeIn_0.4s_ease-out] w-full max-w-5xl mx-auto pt-6 md:pt-10 px-4 md:px-6">
            <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/[0.05] pb-6 relative">
               ...
               <div className="mt-6 md:mt-0 flex flex-wrap bg-[#050505] p-1.5 rounded-xl border border-white/5 shadow-inner gap-1">
                  {['Internship', 'Webinar', 'Offer Letter', 'Member Certificate'].map(cat => (
                    <button key={cat} onClick={() => {setActiveCategory(cat); setMessage(''); setSuccessCode(null);}} className={`px-4 py-2.5 rounded-lg text-[11px] font-bold uppercase tracking-wider transition-all duration-300 ${activeCategory === cat ? 'bg-orange-500/10 border border-orange-500/30 text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.15)]' : 'bg-transparent border border-transparent text-gray-600 hover:text-gray-300'}`}>{cat}</button>
                  ))}
               </div>
            </div>
            ...
            <div className="bg-[#080808]/90 backdrop-blur-3xl border border-white/[0.05] rounded-3xl p-6 md:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden mb-12">
              ...
              <form onSubmit={handleAdminSubmit} className="space-y-5">
                {activeTab === 'single' ? renderUnifiedForm() : (
                      ...
                      {adminSearchResults.map((user, idx) => (
                        <div key={idx} className="bg-[#050505] border border-white/[0.05] p-5 rounded-2xl hover:border-orange-500/30 transition-colors group relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center">
                          ...
                          <div className="flex-shrink-0 w-full md:w-auto">
                            {user.pdfLink ? (
                               <a href={user.pdfLink} target="_blank" rel="noreferrer" className="block text-center px-6 py-3 rounded-xl border border-white/10 bg-white/5 text-gray-300 hover:text-white hover:border-white/30 text-[10px] font-bold uppercase tracking-widest transition-all w-full">View Document</a>
                            ) : (
                               <button onClick={() => handleGenerateCertificate(user.code)} disabled={generatingCode === user.code} className="px-6 py-3 rounded-xl border border-orange-500/50 bg-orange-500/10 text-orange-500 hover:bg-orange-500/20 text-[10px] font-bold uppercase tracking-widest transition-all disabled:opacity-50 w-full">
